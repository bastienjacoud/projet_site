<aside id="ajout_partie_solo">
	...
	<h1 id="titre1_ajout_score_solo">Ajouter le score d'une partie individuelle</h1>
	<?php // marche !
	if(isset($_POST['boutonSubmit'])) // si le bouton pour ajouter le score a été soumis
	{
		...
		$req6="insert into joue_solo values('$idP','$Pseudo')";
		...
		{
			echo '<p id="res_ajout_partie_solo"> Insertion Réussie, vous allez être redirigé sur accueil. <p>';
			echo header("refresh:3;url=index.php");//envoie sur la page d'acceuil au bout de 3s.
		}
		else
		{
			echo '<p id="res_ajout_partie_solo"> Erreur Insertion <p>';
		}
	}
	else
	...
	?>
		<form method="post" action="#">
			...
			<p id="titre2_ajout_score_solo"> Lieu dans lequel s'est déroulé la partie <p>
			<table id="formulaire1_ajout_score_solo">
				<tr>
					<td>
			...
			{?>
				<table id="formulaire2_ajout_score_solo">
					<tr>
						<td>
			...
			if($valeur_bouton == 'nouveau') // si l'on souhaite rentrer un nouveau lieu, on affiche un formulaire permettant de rentrer un nouveau lieu
			{
				...
				{ ?>
					<form method="post" action="#">
					<table id="formulaire3_ajout_score_solo">
						<tr>
							<td>
			...
			<table id="formulaire4_ajout_score_solo">
				<tr>
					<td>
						<label for="idJoueuse"> Pseudo de la joueuse: </label>
					</td>
					<td>
						<select name="Pseudo" id="idJoueuse">
							<option value="" selected="selected"></option>
						<?php 
						$req1="SELECT distinct pseudo FROM joueuses order by pseudo asc";
						$res1=mysqli_query($connexion, $req1);
						while($row1=mysqli_fetch_assoc($res1))
							{ ?>
								<option value="<?php echo $row1['pseudo']; ?>" > <?php echo $row1['pseudo']; ?> </option>
							<?php } ?>
						</select>
						<br>
					</td>
				...
				<tr>
					<td>
						<label for="idJeu"> Jeu auquel la joueuse à joué : </label>
					</td>
					<td>
				...
				<tr>
					<td>
						<label for="Score">Score de la joueuse :</label>
					</td>
					<td>
						...
						<p id="titre3_ajout_score_solo"> La joueuse a-t-elle gagnée ? <p>
			<table id="formulaire5_ajout_score_solo">
				<tr>
					<td>
